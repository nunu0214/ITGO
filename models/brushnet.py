...
def create_mlp(embedding_dim, ca_dim, hidden_dim=None, num_hidden_layers=2, clip_extra_context_tokens=4):
    if num_hidden_layers == 1:
        layers= [nn.Linear(embedding_dim, ca_dim*clip_extra_context_tokens)]
    else:
        if hidden_dim==None:
            hidden_dim = embedding_dim*2
        layers = [nn.Linear(embedding_dim, hidden_dim), nn.GELU()]
        for _ in range(num_hidden_layers - 1):
            layers.append(nn.Linear(hidden_dim, hidden_dim))
            layers.append(nn.GELU())
        layers.append(nn.Linear(hidden_dim, ca_dim*clip_extra_context_tokens))
...
class ImageProjModel(torch.nn.Module):
    ...
    def __init__(self, cross_attention_dim=1024, clip_embeddings_dim=1024, num_hidden_layers=2, clip_extra_context_tokens=4):
        super().__init__()

        self.generator = None
        self.cross_attention_dim = cross_attention_dim
        self.clip_extra_context_tokens = clip_extra_context_tokens
        self.mlp = create_mlp(clip_embeddings_dim, cross_attention_dim,num_hidden_layers=num_hidden_layers)
        self.norm = torch.nn.LayerNorm(cross_attention_dim)

    def forward(self, image_embeds):
        embeds = image_embeds
        clip_extra_context_tokens = self.mlp(embeds).reshape(
...
class BrushNetModel(ModelMixin, ConfigMixin):
    ...
    def from_unet(
        ...
        if if_inpaint_model:
            in_channels = unet.config.in_channels - conditioning_channels
        else:
        ...
        if not load_weights_from_unet:
            # conv_in_condition_weight = torch.zeros_like(brushnet.conv_in_condition.weight)
            conv_in_condition_weight = unet.conv_in.weight
            # conv_in_condition_weight[:, 4:8, ...] = unet.conv_in.weight
            brushnet.conv_in_condition.weight = torch.nn.Parameter(conv_in_condition_weight)
            brushnet.conv_in_condition.bias = unet.conv_in.bias
