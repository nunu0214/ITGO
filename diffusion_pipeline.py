# modified from https://github.com/TencentARC/BrushNet/blob/main/src/diffusers/pipelines/brushnet/pipeline_brushnet.py
import inspect
...
class StableDiffusionITGOPipeline(
    DiffusionPipeline,
    ...
    Args:
        ...
        image_encoder ([`~transformers.CLIPVisionModelWithProjection`]):
            A `CLIPVisionModelWithProjection` to extract sub-image features
        mlp_ca:
            Used to match the dimension of cross-attention layers
    """
    ...
    def encode_image_ca(self, image, device, num_images_per_prompt, output_hidden_states=None):
        dtype = next(self.mlp_ca.parameters()).dtype
        # dtype = torch.float16
        if not isinstance(image, torch.Tensor):
            image = self.feature_extractor(image, return_tensors="pt").pixel_values
        ...
        if output_hidden_states:
            image_enc_hidden_states = self.image_encoder(image, output_hidden_states=True).hidden_states[-2]
            image_enc_hidden_states = self.mlp_ca(image_enc_hidden_states)
            image_enc_hidden_states = image_enc_hidden_states.repeat_interleave(num_images_per_prompt, dim=0)
            uncond_image_enc_hidden_states = self.image_encoder(
                torch.zeros_like(image), output_hidden_states=True
            ).hidden_states[-2]
            uncond_image_enc_hidden_states = self.mlp_ca(uncond_image_enc_hidden_states)
            uncond_image_enc_hidden_states = uncond_image_enc_hidden_states.repeat_interleave(
                num_images_per_prompt, dim=0
            )
            return image_enc_hidden_states, uncond_image_enc_hidden_states
            ...
            image_embeds = self.image_encoder(image).image_embeds
            image_embeds = self.mlp_ca(image_embeds)
            image_embeds = image_embeds.repeat_interleave(num_images_per_prompt, dim=0)
            uncond_image_embeds = torch.zeros_like(image_embeds)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 3. Encode input text image prompt
        ...
        image_ = self.clip_processor(images=original_image, return_tensors="pt").pixel_values
        image_embeds_ca, uncond_image_embeds_ca = self.encode_image_ca(image_ , device, num_images_per_prompt)
        image_embeds_ca, uncond_image_embeds_ca = image_embeds_ca.to(dtype=brushnet.dtype), uncond_image_embeds_ca.to(dtype=brushnet.dtype)
        if self.do_classifier_free_guidance:
            image_embeds_ca = torch.cat([image_embeds_ca, image_embeds_ca])
        ...
        # 4. Prepare image
        if isinstance(brushnet, BrushNetModel):
            masked_image = self.prepare_image(
                image=image,
                width=width,
                height=height,
            ...
            original_mask = (original_mask.sum(1)[:,None,:,:] > 0).to(masked_image.dtype)
        ...
        conditioning_latents = self.vae.encode(masked_image).latent_dist.sample() * self.vae.config.scaling_factor
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    ...
                    brushnet_prompt_embeds = image_embeds_ca.chunk(2)[1]
                else:
                    control_model_input = latent_model_input
                    brushnet_prompt_embeds = image_embeds_ca
