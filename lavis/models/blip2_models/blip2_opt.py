...
class Blip2OPT(Blip2Base):
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image)) # [bs, 257, 1408]

        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(
            image.device
        )
        # pdb.set_trace()
        query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1) #[bs,32,768]
        ...
        inputs_opt = self.opt_proj(query_output.last_hidden_state) #[bs,32,2560]
        ...
        inputs_embeds = torch.cat([inputs_opt, inputs_embeds], dim=1) #把qformer的query token和opt的txt embeding token cat起来
        # [bs,32,2560] + [bs,16,2560]
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            output_text = [text.strip() for text in output_text] #.strip()用于去除字符串首尾的空字符或者换行符
            return output_text
